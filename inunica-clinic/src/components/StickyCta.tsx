...
/**
 * Мобильная нижняя панель. Появляется только после того, как пользователь
 * ушёл с первого экрана (там CTA и так есть), и прячется у подвала, чтобы
 * не перекрывать юридический блок и контакты.
 */
export function StickyCta() {
  ...
  return (
    <div
      ...
      // Панель уезжает вниз — с экрана её нужно убрать и для скринридера.
      aria-hidden={!visible}
    >
      <div className="flex gap-2 px-4 pt-3 pb-[max(0.75rem,env(safe-area-inset-bottom))]">
        ...
        <button
          ...
          onClick={() => open()}
          tabIndex={visible ? undefined : -1}
          className="min-h-[50px] flex-1 cursor-pointer rounded-[2px] bg-accent text-[1rem] font-medium text-paper active:scale-[0.99]"
        >
          Записаться
        </button>
      </div>
