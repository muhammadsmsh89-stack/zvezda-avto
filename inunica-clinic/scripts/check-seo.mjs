...
/*
  Страница 404 из проверки исключена, и это не поблажка: canonical на ней быть
  не должно (страницы по этому адресу нет), а заголовок законно наследуется от
  шаблона сайта. Next выкладывает её тремя файлами — 404.html, 404/index.html
  и _not-found/index.html, — поэтому фильтр ловит все три.
*/
const IS_404 = /(^|\/)(404|_not-found)(\.html$|\/index\.html$)/;
const htmls = walk(OUT)
  .filter(f => f.endsWith('.html'))
  .filter(f => !IS_404.test('/' + path.relative(OUT, f).split(path.sep).join('/')));
