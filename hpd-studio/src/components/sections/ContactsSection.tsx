...
import { MapPin, Clock, Phone, TelegramIcon, VkIcon } from "@/components/ui/Icons";
import { RealPhoto } from "@/components/ui/RealPhoto";
import { contacts, whatsappLink } from "@/lib/contacts";
import { exteriorImage } from "@/lib/media";
...
export function ContactsSection({ className }: { className?: string } = {}) {
  ...
  const rows = [
    { icon: MapPin, label: "Адрес", value: `${contacts.addressFull} · ${contacts.landmark}`, href: mapsSearch, dataEvent: "map_click" },
    ...
    { icon: TelegramIcon, label: "Telegram", value: "hpddetailing", href: contacts.telegramUrl },
    { icon: VkIcon, label: "VK", value: "HPD detailing studio", href: contacts.vkUrl },
  ];
    ...
    <section id="contacts" className={clsx("bg-surface-2/60 pb-20 lg:pb-28", className ?? "pt-20 lg:pt-28")}>
      <Container className="grid grid-cols-1 gap-12 lg:grid-cols-2 lg:items-center lg:gap-16">
        <div>
          ...
          <Reveal delay={0.08}>
            <h2 className="mt-4 text-pretty text-3xl leading-[1.1] text-foreground sm:text-4xl">Ждём вас в студии</h2>
          </Reveal>
          ...
          <Reveal delay={0.3} className="mt-9 flex flex-wrap gap-3">
            <Button href={whatsappLink()} dataEvent="hero_booking_click">Записать автомобиль</Button>
        ...
        <Reveal delay={0.15}>
          ...
          >
            <RealPhoto image={exteriorImage} sizes="(min-width: 1024px) 45vw, 92vw" className="absolute inset-0 h-full w-full" overlay="bottom" />
