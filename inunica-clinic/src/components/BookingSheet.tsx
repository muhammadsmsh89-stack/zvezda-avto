...
type BookingContext = {
  /** subject — контекст, из которого пришёл пользователь: врач, аппарат, процедура. */
...
function Sheet({ subject, onClose }: { subject?: string; onClose: () => void }) {
  ...
  // Escape закрывает, фокус не убегает из диалога, фон не скроллится.
  useEffect(() => {
    ...
    <div className="fixed inset-0 z-[90] flex items-end justify-center sm:items-center">
      ...
      >
        <button
          ...
          onClick={onClose}
          aria-label="Закрыть"
          className="absolute right-4 top-4 grid h-11 w-11 cursor-pointer place-items-center text-ink-mute transition-colors duration-200 hover:text-ink"
        >
...
function Form({
  ...
  return (
    <form onSubmit={submit} noValidate>
      ...
      {error && (
        <p id={errId} role="alert" className="mt-5 text-[1rem] text-accent-deep">
          {error}
        </p>
      )}
      ...
      <p id={noteId} className="mt-5 text-[0.9375rem] leading-relaxed text-ink-mute">
      ...
      <p className="mt-6 border-t border-line pt-6 text-[1rem] text-ink-soft">
        ...
        <a
          ...
          className="whitespace-nowrap text-ink underline underline-offset-4"
        >
          {site.phone.display}
        </a>
...
function Success({ onClose }: { onClose: () => void }) {
  return (
    <div className="py-6">
      ...
      <h2 className="font-display mt-4 text-[1.875rem] text-ink">
        Форма готова к подключению
      </h2>

      {/*
        Честность важнее красивого «Спасибо!»: бэкенда у статического экспорта
        нет, поэтому мы не изображаем отправку. Пользователю сразу даём
        рабочий канал связи, а владельцу сайта — понятную точку интеграции.
      */}
      <p className="mt-5 text-[1.125rem] leading-[1.65] text-ink-soft">
        Данные не были отправлены: на сайте пока не подключён приём заявок.
        Чтобы записаться прямо сейчас, позвоните или напишите — администратор
        подберёт врача и время.
      </p>

      <div className="mt-8 flex flex-col gap-3">
        <a
          href={site.phone.href}
          className="inline-flex min-h-[52px] items-center justify-center rounded-[2px] bg-plum px-7 text-[1rem] font-medium text-shell transition-colors duration-200 hover:bg-plum-soft active:scale-[0.98]"
        >
        ...
        <a
          ...
          className="inline-flex min-h-[52px] items-center justify-center rounded-[2px] border border-ink/25 px-7 text-[1rem] font-medium text-ink transition-colors duration-200 hover:border-ink hover:bg-ink/[0.04] active:scale-[0.98]"
        >
          Написать в WhatsApp
        </a>
      </div>
