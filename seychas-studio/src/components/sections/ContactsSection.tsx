...
import { MapPin, Clock, Phone, WhatsAppIcon, InstagramIcon, VkIcon } from "@/components/ui/Icons";
...
export function ContactsSection({ className }: { className?: string } = {}) {
  ...
  const rows = [
    ...
    { icon: Clock, label: "График", value: contacts.hoursNote },
    { icon: InstagramIcon, label: "Instagram", value: contacts.instagramHandle, href: contacts.instagramUrl },
    { icon: VkIcon, label: "VK", value: "SEYCHAS студия красоты Тула", href: contacts.vkUrl },
  ];
      ...
      <Container className="grid grid-cols-1 gap-12 lg:grid-cols-2 lg:items-center lg:gap-16">
        <div>
          ...
          <Reveal delay={0.3} className="mt-9 flex flex-wrap gap-3">
            <Button href={contacts.dikidiUrl}>Записаться</Button>
            <Button variant="secondary" href={whatsappLink()} icon={<WhatsAppIcon className="h-4 w-4" />}>
              WhatsApp
            </Button>
          </Reveal>
        </div>
