...
export function Header() {
  ...
  return (
    ...
    >
      <div
        className={`mx-auto flex max-w-[85rem] items-center justify-between gap-4 px-5 transition-all duration-300 sm:px-8 lg:px-16 ${
          scrolled ? "py-3" : "py-4"
        }`}
      >
        <Link href="/" className="group flex items-center gap-2 shrink-0" aria-label="На главную">
          <Wordmark />
        </Link>
        ...
        <button
          ...
          className="flex h-11 w-11 items-center justify-center rounded-full border border-border text-foreground lg:hidden shrink-0"
        >
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            ...
            <Button href={whatsappBookingLink()} variant="ghost-light" className="mb-6 w-full justify-center" size="lg" onClick={() => setOpen(false)}>
              {ctaLabels.primary}
            </Button>
          </motion.div>
        )}
