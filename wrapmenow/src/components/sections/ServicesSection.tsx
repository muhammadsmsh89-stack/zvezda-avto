    ...
    <section className="bg-background py-20 lg:py-28">
      <Container>
        ...
        <div className="mt-12 grid grid-cols-1 gap-6 lg:grid-cols-2">
          {list.map((service, i) => (
            <Reveal key={service.slug} delay={(i % 2) * 0.08} className={clsx(i === 0 && "lg:col-span-2")}>
              <article className="group relative overflow-hidden rounded-[1.75rem] border border-border bg-surface">
                <div className={clsx("relative w-full overflow-hidden", i === 0 ? "aspect-[21/9]" : "aspect-[4/3]")}>
                  ...
                  <span className="absolute left-6 top-6 rounded-full bg-deep/60 px-2.5 py-1 font-[var(--font-display)] text-sm font-medium text-foreground backdrop-blur-sm">
                    {service.number}
                  </span>
                </div>
                <div className="p-6 sm:p-7">
                  <h3 className="text-xl font-medium text-foreground sm:text-2xl">{service.title}</h3>
                  <p className="mt-2 max-w-md text-sm leading-relaxed text-muted">{service.short}</p>
                  <div className="mt-5 flex flex-wrap items-center gap-3">
                    {service.fromPrice && <span className="text-sm font-semibold text-accent">{service.fromPrice}</span>}
        ...
        {full && (
          <Reveal delay={0.15} className="mt-14 flex justify-center">
            <Button size="lg" href={whatsappLink()} dataEvent="hero_calc_click">
              {ctaLabels.primary}
            </Button>
          </Reveal>
        )}
