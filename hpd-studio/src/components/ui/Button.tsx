...
const variants: Record<Variant, string> = {
  ...
  "ghost-light":
    "bg-transparent text-foreground border border-foreground/30 hover:border-foreground hover:bg-foreground/10",
};
...
export function Button({
  ...
  if (href) {
    if (href.startsWith("/")) {
      return (
        <Link href={href} className={classes} data-event={dataEvent}>
          {content}
        </Link>
      );
    }
  ...
  return (
    <button type={type} onClick={onClick} disabled={disabled} className={classes} data-event={dataEvent}>
      {content}
    </button>
  );
}
...
export function TextLink({
  ...
  const classes = clsx(
    ...
    tone === "dark" ? "text-foreground hover:text-foreground/70" : "text-foreground hover:text-accent",
    className
  );
  const content = (
    <>
