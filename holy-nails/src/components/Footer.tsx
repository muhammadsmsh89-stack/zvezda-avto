...
import { Phone, MapPin, Clock, WhatsAppIcon, TelegramIcon, VkIcon, InstagramIcon, Star } from "@/components/ui/Icons";
...
const socials = [
  ...
  { icon: VkIcon, label: "ВКонтакте", href: contacts.vkUrl },
      ...
      <Container className="py-10 lg:py-12">
        <div className="flex flex-col gap-10 sm:flex-row sm:justify-between">
          <div className="max-w-xs">
            <Wordmark dark className="text-base" />
            ...
            <div className="mt-5 flex items-center gap-3">
              {socials.map((s) => (
                <a
                  ...
                  aria-label={s.label}
                  className="flex h-9 w-9 items-center justify-center rounded-lg border border-background/20 text-background/80 transition-colors hover:border-background hover:text-background"
                >
        ...
        <div className="mt-8 flex flex-col gap-2 border-t border-background/15 pt-5 text-xs text-background/55 sm:flex-row sm:items-center sm:justify-between">
          <p>{studio.name} · {studio.city}. Актуальную стоимость услуг уточняйте при записи.</p>
          <p>ОГРНИП {legal.ogrnip} · ИНН {legal.inn}</p>
        </div>
      </Container>
    </footer>
  );
